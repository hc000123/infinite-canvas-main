...
- 验收步骤：
  ...
  2. 确认画布顶部出现 P01 / P02 / P03 生产包切换条；即使是只有分镜镜头、尚未真正导入生产包节点组的旧画布，也应看到 P 包入口。
  3. 点击任一生产包，确认画布视图聚焦到该生产包节点组，右侧内容检查器切换到生产包绑定信息。
  4. 确认同包节点显示 `Pxx · 原剧本 / 引用资产 / 提示词 / 视频配置 / 视频结果` 等轻量标签，不遮挡节点主体内容。
  5. 对一个生产包生成视频，确认新视频结果作为该包的新版本出现，旧视频结果节点仍保留。
  6. 在右侧“视频版本”中预览、导出、设为当前和隐藏版本，确认当前采用版本只有一个且青色高亮。
  7. 点击“生成新版本”，确认使用当前生产包的视频配置节点进入生成，不从画布承接页直接生成。
  8. 对有上一包当前版本的生产包启用尾帧衔接，确认右侧显示上一包版本来源；切换上一包当前版本后，后续包显示可能需要重新检查。
- 明确不应发生：
  - 不新建第二套画布页面。
