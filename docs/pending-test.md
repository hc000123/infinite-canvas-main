...
- 本次实现：
  ...
  - 本集生产流程页改为单集制作页：顶部保留项目路径、当前集标题、当前阶段和返回项目按钮；下方用剧本 / 导演分析 / 资产提取 / 分镜 / 画布承接模块 Tab 组织主流程，主体改为摘要 + 筛选 / 操作 + 长条表，详细剧本、分析、资产、分镜和承接内容进入详情抽屉查看。
...
- 验收步骤：
  ...
  12. 打开任意本集生产流程页，确认页面是单集制作页结构：顶部显示项目路径、集标题、当前阶段和返回项目按钮；模块 Tab 包含剧本、导演分析、资产提取、分镜、画布承接；主区域以摘要、筛选 / 操作和长条表展示，点击“查看”后在详情抽屉中查看完整内容。
