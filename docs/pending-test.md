...
- 本次实现：
  - 在现有无限画布顶部增加轻量生产包切换条，按现有 `shotGroup` 显示 P01、P02、P03 等生产包状态。
  - 点击生产包会聚焦到该生产包对应的现有画布节点组，并在右侧“内容”检查器显示该包绑定信息。
  - 由生产包导入的节点显式记录 `productionPackageId`、生产包标题和节点角色；旧节点继续兼容 `shotGroupId` / `storyboardShotGroupId`。
...
- 验收步骤：
  ...
  2. 确认画布顶部出现 P01 / P02 / P03 生产包切换条，当前包用青色强调，并显示待生成、已生成、有多版本、缺资产或已采用等短状态。
