...
整体验收总入口：先按 `docs/acceptance-checklist.md` 执行 P0 主流程验收，再按本文档明细抽样回归。当前已完成 HTTP 路由、AI 配置模板、项目 / 分集 / 本集生产流程 / 承接画布入口浏览器交互和前端逻辑测试层验收，结论为有条件通过；画布拖拽上传、素材弹窗、配置包文件导入、真实 AI 调用和重启后版本显示仍需人工或 E2E 补验。
...
- 本次实现：
  ...
  - 仅导入 AI 配置白名单字段，例如渠道模式、Base URL、API Key、默认模型、模型列表、视频协议、Seedance 模型 / Endpoint、系统提示词等。
- 验收步骤：
