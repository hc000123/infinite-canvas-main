...
- 验收步骤：
  ...
  6. 打开 `/projects/:id/agents`、`/projects/:id/agent` 或任意本集生产流程页，确认左侧栏仍作为项目区导航保留。
  7. 在 `/projects` 确认项目名称、预设和项目说明可以完整查看，卡片视觉与参考方向一致。
  8. 打开 `/canvas`，确认画布卡视觉统一，画布标题和预设摘要可以完整查看。
  9. 打开任意 `/projects/:id`，确认项目详情首屏与项目工作台视觉一致，生产主线、总览和素材引用卡片没有明显旧样式断层。
  10. 打开任意本集生产流程页，确认步骤卡、折叠流程、阶段产物、场次推进和写入预览统一为控制台面板风格。
- 明确不应发生：
