...
- 验收步骤：
  ...
  5. 点击任意项目进入 `/projects/:id`，确认左侧栏仍固定显示，可继续收起 / 展开，并且顶部旧导航不再重复出现。
  6. 在 `/projects/:id` 确认首屏是分集工作台布局，顶部不重复显示 “AI · 画布” 品牌区，能看到项目内导航、紧凑整剧制作进度、分集筛选和分集表，不再显示旧的生产主线 Tabs 首屏。
  7. 点击分集表中的“进入 / 查看”，确认可以进入对应本集生产流程。
  8. 点击“编辑项目”，确认可以修改项目名称和项目说明；点击“新建分集”，确认仍打开导入本集剧本弹窗。
  9. 打开 `/projects/:id/agents`、`/projects/:id/agent` 或任意本集生产流程页，确认左侧栏仍作为项目区导航保留。
  10. 在 `/projects` 确认项目名称、预设和项目说明可以完整查看，卡片视觉与参考方向一致。
  11. 打开 `/canvas`，确认画布卡视觉统一，画布标题和预设摘要可以完整查看。
  12. 打开任意本集生产流程页，确认页面是单集制作页结构：顶部显示项目路径、集标题、当前阶段和返回项目按钮；模块 Tab 包含剧本、导演分析、资产提取、分镜生产包、画布承接；主区域以摘要、筛选 / 操作和长条表展示，点击“查看”后在详情抽屉中查看完整内容。
  13. 点击“资产提取”模块，确认页面顶部显示项目名 / 当前集 / 资产提取，摘要显示角色、场景、道具、服装和缺素材数量；左侧长条表支持全部、缺素材、已绑定、待生成、角色、场景、道具、服装筛选；点击单行的“绑定”或“生成”后，右侧资产处理抽屉切换到对应 Tab。
  14. 点击“分镜生产包”模块，确认页面顶部显示项目名 / 当前集 / 分镜生产包，摘要显示剧情段落数、生产包数、包内镜头数、超时包数量和缺资产包数量；主列表按剧情段落分组展示生产包，超时包显示风险提示和拆分操作；点击生产包后右侧抽屉可在包内镜头、原剧本、提示词、资产之间切换。
  15. 点击“画布承接”模块，确认页面顶部显示项目名 / 当前集 / 画布承接，5 步链路高亮“导入到画布”；摘要显示已确认生产包、已导入、待导入、缺资产和承接画布数量；左侧表格展示每个生产包的导入状态，右侧预览展示导入后会创建的画布节点，并且页面没有“开始生成视频”按钮。
- 明确不应发生：
