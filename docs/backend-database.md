...
通用 Skill 稳定身份表。记录名称、说明、`system` / `project` 所有者、创建用户、项目归属、固定生产阶段 `stage_key`、启用状态和当前推荐版本；不保存版本正文。Definition 以 ID 为唯一身份，所有者、项目和名称只作普通查询索引，允许同名独立 Skill。System Skill 只允许管理员写，Project Skill 按创建用户与项目隔离；复制 System Skill 会创建新的 Project Definition 和 Draft，不改变源记录。
