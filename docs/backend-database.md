...
通用 Skill 稳定身份表。记录名称、说明、固定生产阶段 `stage_key`、启用状态和当前推荐版本；不保存版本正文。当前只允许 `system` owner，`owner_user_id` 和 `owner_project_id` 始终为空，全部 Definition / Version 统一由管理员在后台管理。Definition 以 ID 为唯一身份；启用、已发布且契约匹配的精确版本对所有已登录账号和项目可见，项目侧只选择要冻结的精确版本，不创建、复制或管理项目专属 Skill。

### skill_versions

Skill 不可变版本表。`skill_id + version` 唯一；分别保存 Manifest、逻辑文件、输入契约、输出契约、质量门、内容哈希和评测摘要。外部文件夹导入版本还保存 `source_kind`、完整文件夹哈希 `source_hash`、仅对文件夹导入填充的可空 `source_identity`、规范化 ZIP 快照 `source_archive_blob`、文件索引和导入元数据；`skill_id + source_identity` 唯一，在数据库层阻止同一 Definition 并发导入重复内容。压缩快照只在服务端持久化，不进入普通 JSON 响应。Definition / Version 与导入审计在同一事务创建。只有 Draft 可原地编辑；发布、推荐、归档和停用相互独立，运行只冻结精确 Version ID 与内容哈希。未引用 Draft 和从未发布的 Definition 可安全删除，已发布、已归档、已评测、已绑定或被 Workflow / Agent / Invocation 引用的记录禁止物理删除。所有者收口不改写稳定 Definition ID / Version ID，也不中断评测、审计、Workflow / Agent 和历史 Invocation 的引用关系。

### workflow_stage_skill_bindings

生产工作流阶段到通用 System Skill Version 的消费端绑定。绑定由管理员统一维护；项目运行时可明确选择契约匹配的已发布精确版本，解析时会验证 `workflow.stage.<stage>` capability。

### skill_evaluations / skill_audit_logs

通用 Skill 的冻结试运行、同输入对比，以及创建、编辑、发布、推荐、回滚、归档、停用和安全删除审计。试运行不写正式工作流阶段或业务资产；所有者收口后的历史评测与审计记录仍按原 Skill / Version ID 保留。
