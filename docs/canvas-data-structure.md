...
- `ShotGroup.assetRefs` / `audioRefs` / `productionBibleRefs`：记录生成镜头组可用的图片、参考视频、音频和设定库输入。引用素材时继续保存素材版本快照；M6.8 起素材引用可标记 `source: "asset_breakdown" | "independent"`，用于区分剧本拆解资产和独立生图工作台资产；M6.9.7 自动参考资产还会使用 `agent_asset_extractor / manual / independent_image_brief / production_bible / asset_breakdown / unknown` 来源标记。
...
- 镜头组参考素材：视频配置节点会同步保存 `referenceAssets / referenceRoles / referenceOrder`，参考素材节点和 `referenceAssets` 都继续保存 `assetVersion` 快照；同一素材不会重复写入。M6.9.7 起，镜头组加入画布前会基于同 `projectId + episodeId` 的本集生图需求和 Brief 结果生成自动参考候选，记录 `sourceType / sourceLabel / isAutoMatched / isPrimaryReference / assetBreakdownItemId / imageBriefId / matchReasons / assetVersion`。来源不明确的候选会标记 `sourceType: "unknown"` 和“来源待确认”，默认不选中。
- M6.9 追溯链：`asset_extractor` run 写入本集生图需求，需求创建 Brief 并生成参考图；`storyboard_director` run 写入 `StoryboardTableShot`，镜头组继承分镜 trace；镜头组加入画布时，视频配置节点把 `agentRunId / agentConfigId / agentConfigVersion`、本集字段、镜头组字段、Brief/资产需求引用和素材 `assetVersion` 一并保存。整条链路仍只保存本地草案、确认结果和节点 metadata，不接真实 LLM，不自动生成图片或视频。
