...
1. 用户在统一工作台启动阶段。
2. 分镜提示词阶段由前端解析当前场次 / 生成批次的图片，按固定优先级上传最多 9 张参考图，并取得一次性批次 ID。
3. 后端根据启动时已经验证的服务端执行器模式创建 Agent Run，写入 `Executor` 与图片清单。
4. Worker 领取任务并续租；API 任务选择后台渠道并计费，CLI 任务通过 `-i` 附加暂存图片并启动受控子进程，且不计应用算力点。
5. 两种执行器都返回同一格式的文本与结构化草案，artifact 记录图片引用业务元数据。
6. 现有质量门、审核哈希、映射预览、应用本地和回执流程继续执行。
7. 停止、超时、重试、刷新恢复和服务退出使用同一状态机。
...
使用宿主机 Codex 登录态，从统一页面完成一次导演与美术阶段和一次带参考图的分镜提示词阶段，检查 CLI 是否基于图片可见内容生成提示词，并核对队列、图片清单、日志、质量门、审核、资产 / 生产包映射、刷新恢复、停止与重试。该验收可能消耗 Codex 账户额度，但不扣应用算力点。
