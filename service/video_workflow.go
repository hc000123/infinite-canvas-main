...
func workflowStagePrompts(run model.WorkflowRun, stageID string, input model.WorkflowArtifact) (string, string) {
	if stageID == WorkflowStageArtDesign {
		return "你是影视导演与美术设定师。只输出 JSON，包含 directorSummary 和 items；每个 item 必须有 id、kind、name、prompt。不得改变剧本事实。", fmt.Sprintf("工作流版本：%s\n请根据以下已确认生产剧本生成角色、场景、道具设定：\n%s", run.WorkflowVersion, run.ScriptSnapshot)
	}
	return "你是 Seedance 分镜师。只输出 JSON，包含 shots；每个 shot 必须有 id、sceneId、prompt、duration，可选 dialogue。duration 必须为 4–15 秒，素材引用使用 @图N。", fmt.Sprintf("工作流版本：%s\n生产剧本：\n%s\n\n已批准美术产物：\n%s", run.WorkflowVersion, run.ScriptSnapshot, input.ContentJSON)
}
