...
func TestPublishedWorkflowSkillCannotBePatched(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodPatch, "/api/v1/admin/workflow-skill-versions/workflow-skill-version-art-2.0.1", strings.NewReader(`{"files":{"SKILL.md":"changed"},"contract":{}}`))
	request = request.WithContext(service.WithUser(context.Background(), model.AuthUser{ID: "admin-1", Role: model.UserRoleAdmin}))
	recorder := httptest.NewRecorder()

	AdminUpdateWorkflowSkillVersion(recorder, request, "workflow-skill-version-art-2.0.1")
