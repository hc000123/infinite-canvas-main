...
func TestPublishedWorkflowSkillCannotBePatched(t *testing.T) {
	...
	resolved, err := service.ResolvePublishedWorkflowSkill(service.WorkflowSkillStageArt, "")
	if err != nil {
		t.Fatal(err)
	}
	request := httptest.NewRequest(http.MethodPatch, "/api/v1/admin/workflow-skill-versions/"+resolved.Version.ID, strings.NewReader(`{"files":{"SKILL.md":"changed"},"contract":{}}`))
	request = request.WithContext(service.WithUser(context.Background(), model.AuthUser{ID: "admin-1", Role: model.UserRoleAdmin}))
	recorder := httptest.NewRecorder()

	AdminUpdateWorkflowSkillVersion(recorder, request, resolved.Version.ID)
