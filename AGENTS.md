...
- 日常开发写完代码默认不执行语法检查、构建或测试；当用户明确要求“全面验收”“完整检查”“发布前检查”或指定执行时，应按要求运行对应检查。
- 不要改无关文件，不要顺手重构。
...
- 对话历史不是正式事实。发生冲突时依次以用户当前要求、本文件、当前代码和运行证据、有效决策、其他仓库文档、岗位文件、对话历史判断。
...
- 每次上线或发版本前先完整读取 `docs/release/README.md`，以该文件作为提交前后冒烟、版本整理、tag、推送、GitHub Actions、阿里云手工部署、云端复验和回滚的唯一操作清单；流程变化时必须同步更新该文件。
...
- 用户侧全局导航不得为同一业务能力设置重复一级入口；项目中心是唯一项目一级入口，分集制作从项目内进入 Workflow，旧 `/agent` 仅作为兼容跳转保留。
- `codex/model-configuration-management` 分支中的模型/API 配置改造保持隔离；主线日常提交、发版或“提交全部改动”时不得合并、cherry-pick 或以其他方式带入该分支提交，除非用户之后再次明确授权。
...
- 视频工作流中，上一镜尾帧只能作为下一连续镜头的 `continuity_reference` 普通参考图，用于理解剧情从上一画面之后继续；不得标记或提交为首帧，不得要求第一帧复刻，也不得据此重新诠释画风、材质或角色设定。
- 即梦 CLI 的生成子命令即使素材不存在、参数非法并返回 `gen_status=fail`，也可能创建任务并扣除积分；参数级调试只使用帮助、项目假 CLI 和单元测试，未经用户明确确认不得用真实生成子命令做“无付费”验证。
