...
export function defaultAgentConfigs(now = defaultUpdatedAt): AgentConfig[] {
    return [
        buildDefaultAgentConfig({
            ...
            userPromptTemplate:
                "项目：{projectTitle}\n集数：{episodeTitle}\n\n原始剧本：\n{scriptSnapshot}\n\n请输出一版可直接确认提交的完整优化剧本，并同步生成结构化 JSON。\n输出要求：\n1. 顶层必须是 JSON 对象，只包含 productionScript 和 structuredScript。\n2. productionScript 必须按每场使用如下结构重写：场次编号 / 地点 / 时间 / 内外 / 出场人物 / 场记 / 动作视觉 / 对白。\n3. structuredScript 必须包含 episodeTitle、summary、characters、scenes；每个 scene 包含 sceneId、location、timeOfDay、space、characters、sceneNote、beats、assets。\n4. beats 只能使用 action / dialogue / visual / note；dialogue 必须保留 speaker 和 text。\n5. 必须把原稿里简略的动作补成可拍的视觉段落，补足人物站位、视线、表情、手部动作、重心、道具状态、环境声音和连续性。\n6. 台词原意和说话顺序不变；原台词文字尽量保留，但可以在台词前后增加动作和情绪承接。\n7. 不允许只复制原文、只改标点或只做轻微润色；如果原稿已经短，也必须输出更完整的标准生产稿。\n8. 不做导演讲戏，不输出资产清单，不输出分镜提示词。",
            inputVariables: [
