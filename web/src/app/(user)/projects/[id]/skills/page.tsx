...
export default function ProjectSkillsPage() {
    ...
    const draftMutation = useMutation({
        mutationFn: async () => {
            ...
            const sourcePackage = sourceVersion ? (detailQuery.data?.version.id === sourceVersion.id ? detailQuery.data.package : (await fetchProjectSkillVersion(token, sourceVersion.id)).package) : emptyPackage();
            return createProjectSkillVersion(token, item.skill.id, { version: sourceVersion ? nextPatch(sourceVersion.version) : "1.0.0", package: { ...sourcePackage, contentHash: "" } });
        },
        onSuccess: async (version) => { await invalidate(); setActiveVersionId(version.id); message.success(`已创建草稿 v${version.version}`); },
