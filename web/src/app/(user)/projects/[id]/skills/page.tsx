...
export default function ProjectSkillsPage() {
    ...
    return <main className="studio-shell h-full overflow-auto text-[var(--studio-text-primary)]">
        ...
        <SkillFolderImport open={Boolean(folderImportMode)} token={token} scope="project" projectId={projectId} skillId={folderImportMode === "version" ? activeItem?.skill.id : undefined} onCancel={() => setFolderImportMode("")} onImported={async (skillId, versionId) => { setFolderImportMode(""); await invalidate(); if (skillId) setActiveSkillId(skillId); if (versionId) setActiveVersionId(versionId); }} />
