...
export default function ProjectSkillsPage() {
    ...
    useEffect(() => {
        setOpenStageKeys(resolveOpenSkillStageKeys(stageGroups, activeItem?.skill.id || "", Boolean(search.trim() || ownerFilter)));
    }, [activeItem?.skill.id, ownerFilter, search, stageGroups]);
    ...
    const deleteSkillMutation = useMutation({ mutationFn: (skillId: string) => deleteProjectSkill(token, skillId), onSuccess: async (_, deletedSkillId) => { if (activeSkillId === deletedSkillId) { setActiveSkillId(""); setActiveVersionId(""); } await invalidate(); message.success("从未发布的 Skill 已删除"); }, onError: mutationError });
    ...
    return <main className="studio-shell h-full overflow-auto text-[var(--studio-text-primary)]">
        <div className="mx-auto flex w-full max-w-[1900px] flex-col gap-5 px-5 py-7 lg:px-8">
            <header className="border-b border-[var(--studio-border-subtle)] pb-5">
                <Link href={`/projects/${projectId}`} className="text-xs text-[var(--studio-text-muted)] transition hover:text-[var(--studio-accent)]">返回项目 · {project.title}</Link>
                <div className="mt-3 flex flex-wrap items-end justify-between gap-4"><div><div className="flex items-center gap-2"><Library className="size-6 text-[var(--studio-accent)]" /><h1 className="text-3xl font-semibold">Skill 管理</h1></div><p className="mt-2 max-w-4xl text-sm leading-6 text-[var(--studio-text-secondary)]">导入单个 SKILL.md 或完整外部 Skill 文件夹，选择所属阶段后即可独立试跑；通过后由固定生产阶段、画布、图片与 API 共用同一冻结版本。</p></div><Button type="primary" onClick={() => setFolderImportMode("new")}>导入项目 Skill</Button></div>
            </header>
            ...
            {skillsQuery.isLoading ? <Skeleton active paragraph={{ rows: 14 }} /> : !activeItem ? <Card className="studio-panel" variant="borderless"><Empty description="没有匹配的 Skill" /></Card> : <div className="grid grid-cols-[minmax(240px,280px)_minmax(460px,1fr)_minmax(240px,280px)] items-start gap-4 xl:overflow-hidden max-[1120px]:grid-cols-[240px_minmax(440px,1fr)] max-[840px]:grid-cols-1">
                <Flex vertical gap={12} className="min-h-0 xl:max-h-[calc(100dvh-250px)] xl:overflow-y-auto xl:pr-1">
                    <Card className="studio-panel" variant="borderless" title={`注册表 · ${items.length}`}>
                        <Collapse
                            ghost
                            ...
                            items={stageGroups.map((group) => ({
                                key: group.key,
                                label: <Flex justify="space-between" align="center" gap={8} wrap><Typography.Text strong>{group.label}</Typography.Text><Space size={4} wrap><Tag>{group.totalCount} 个</Tag>{group.systemCount ? <Tag color="blue">System {group.systemCount}</Tag> : null}{group.projectCount ? <Tag color="gold">Project {group.projectCount}</Tag> : null}</Space></Flex>,
                                children: <Flex vertical gap={8}>{group.items.map((item) => <SkillCard key={item.skill.id} item={item} active={item.skill.id === activeItem.skill.id} deleting={deleteSkillMutation.isPending && deleteSkillMutation.variables === item.skill.id} onClick={() => { setActiveSkillId(item.skill.id); setActiveVersionId(""); }} onDelete={item.skill.ownerType === "project" ? () => deleteSkillMutation.mutate(item.skill.id) : undefined} />)}</Flex>,
                            }))}
                        />
                    </Card>
        ...
        <SkillTrialPanel open={trialOpen} token={token} scope="project" versionId={activeVersionId} executorKind={detailQuery.data?.package.manifest.executorKind} onCancel={() => setTrialOpen(false)} onCompleted={async () => { await invalidate(); }} />
...
function SkillCard({ item, active, deleting, onClick, onDelete }: { item: SkillAdminItem; active: boolean; deleting: boolean; onClick: () => void; onDelete?: () => void }) {
    const manifest = item.recommendedPackage?.manifest;
    return <div className={`relative rounded-lg border transition ${active ? "border-[var(--studio-accent)] bg-[var(--studio-accent-soft)]" : "border-[var(--studio-border-subtle)] bg-[var(--studio-panel-muted-bg)] hover:border-[var(--studio-border-strong)]"}`}><button type="button" onClick={onClick} className="w-full p-3 text-left"><Flex justify="space-between" gap={8} className={onDelete ? "pr-8" : undefined}><Typography.Text strong>{item.skill.name}</Typography.Text><Tag>{item.skill.ownerType === "system" ? "System" : "Project"}</Tag></Flex><Typography.Text type="secondary" className="mt-1 block line-clamp-2 text-xs">{item.skill.summary}</Typography.Text><Flex gap={4} wrap className="mt-2">{manifest?.capabilities.slice(0, 2).map((value) => <Tag key={value} color="blue">{value}</Tag>)}</Flex><Typography.Text type="secondary" className="mt-2 block text-[11px]">{manifest ? `${manifest.inputArtifactTypes.join(", ")} → ${manifest.outputArtifactTypes.join(", ")}` : "尚未设置推荐版本"}</Typography.Text></button>{onDelete ? <div className="absolute right-2 top-2"><Popconfirm title={`删除“${item.skill.name}”？`} description="只有从未发布、没有引用的 Skill 才能删除。" okText="确认删除" cancelText="取消" okButtonProps={{ danger: true }} onConfirm={onDelete}><Button aria-label={`删除 Skill ${item.skill.name}`} danger type="text" size="small" icon={<DeleteOutlined />} loading={deleting} onClick={(event) => event.stopPropagation()} /></Popconfirm></div> : null}</div>;
}
