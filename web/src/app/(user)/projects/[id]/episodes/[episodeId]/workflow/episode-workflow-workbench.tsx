...
export function EpisodeWorkflowWorkbench({ episodeId, projectId, returnHref, returnLabel }: { episodeId: string; projectId: string; returnHref: string; returnLabel: string }) {
    ...
    const shotAutomation = useWorkflowShotAutomation({ artifact: breakdown.artifact, episodeId, gate: breakdown.gate, onApplied: workbench.refreshRemote, projectId, stage: breakdown.stage });
    const referenceImages = useMemo(() => workflowReferenceImages(assets, projectId, episodeId), [assets, episodeId, projectId]);
        ...
        <div className={cn("grid min-h-0 flex-1 grid-cols-1", showsQueue ? "xl:grid-cols-[168px_252px_minmax(460px,1fr)_320px]" : "xl:grid-cols-[168px_minmax(0,1fr)]") }>
            ...
            <section className="thin-scrollbar min-h-0 overflow-y-auto bg-[var(--studio-workspace-bg)] px-4 py-4 xl:px-6">
                <div className={cn("mx-auto", ["asset-extraction", "asset-production"].includes(workbench.routeState.stage) ? "max-w-7xl" : "max-w-5xl")}>
                    ...
                    {workbench.routeState.stage === "video" ? <div className="space-y-4">{workbench.selectedPackage ? <WorkflowShotEditor mode="video" item={workbench.selectedPackage} packages={workbench.packages} /> : null}<WorkflowDeliveryPanel assets={assets} episodeId={episodeId} packages={workbench.packages} projectId={projectId} scriptSnapshot={workbench.scriptSnapshot} workflowRunId={workbench.detail?.run.id} /></div> : null}
                </div>
            </section>
