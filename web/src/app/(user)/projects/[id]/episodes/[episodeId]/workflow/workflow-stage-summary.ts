...
export function summarizeWorkflowStages(input: WorkflowStageSummaryInput): WorkflowStageView[] {
    ...
    const breakdown = remoteStatus(status("shot-breakdown"), input.workerReady, done(imagePrompt));
    ...
    return definitions.map((stage): WorkflowStageView => {
        ...
        if (stage.key === "video") {
            ...
            return { ...stage, status: active, count: packages ? `${generated}/${packages}` : undefined, blockingReason: active === "blocked" ? workerReason(input.workerReady, done(imagePrompt)) : undefined };
        }
