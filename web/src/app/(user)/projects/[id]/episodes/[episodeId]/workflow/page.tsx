...
function StageBadge({ status }: { status: string }) { const blocked = ["blocked", "failed", "rejected"].includes(status); const done = ["approved", "applied", "complete"].includes(status); const labels: Record<string, string> = { approved: "已批准", applied: "已应用", blocked: "被阻断", complete: "已完成", failed: "失败", idle: "未开始", queued: "排队中", ready: "可开始", rejected: "已退回", review: "待审核", running: "运行中" }; return <span className={cn("inline-flex items-center gap-1.5 rounded-md border px-2 py-1 text-xs", blocked ? "border-[var(--studio-warning)] text-[var(--studio-warning)]" : done ? "border-[var(--studio-success)] text-[var(--studio-success)]" : "border-[var(--studio-border-subtle)] text-[var(--studio-text-secondary)]")}>{blocked ? <CircleAlert className="size-3" /> : done ? <CheckCircle2 className="size-3" /> : <Clock3 className="size-3" />}{labels[status] || status}</span>; }
