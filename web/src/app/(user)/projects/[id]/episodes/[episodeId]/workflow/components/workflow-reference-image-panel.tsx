...
export function WorkflowReferenceImagePanel(props: { images: WorkflowReferenceImage[]; item: ProductionPackage }) {
    ...
    return <section className="rounded-md border border-[var(--studio-border-subtle)] bg-[var(--studio-panel-bg)] p-4 shadow-[var(--studio-shadow)]">
        <div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2 text-sm font-semibold"><Images className="size-4 text-[var(--studio-accent)]" />本镜参考图</div><p className="mt-1 text-xs leading-5 text-[var(--studio-text-muted)]">先确认分镜，再为本镜选择或导入参考图；每张图都带用途、资产编号和冻结版本。</p></div><div className="flex items-center gap-2"><span className="text-xs text-[var(--studio-text-secondary)]">已选 {bindings.length}/9</span><Dropdown menu={{ items: [{ key: "local", label: "从本地导入" }, { key: "library", label: "从素材库导入" }], onClick: ({ key }) => key === "local" ? fileInput.current?.click() : setPickerOpen(true) }}><Button size="small" icon={<Upload className="size-3.5" />} loading={importing}>导入参考图</Button></Dropdown></div></div>
