...
export function WorkflowVersionEditor({ item, projectId, skillOptions, agents, onVersionChange }: { item?: WorkflowRegistryItem; projectId: string; skillOptions: SkillOption[]; agents: AgentRegistryItem[]; onVersionChange: (versionId: string, pkg?: WorkflowPackage) => void }) {
    ...
    const publishedAgents = agents.flatMap((agent) => agent.versions.filter((candidate) => candidate.status === "published").map((candidate) => ({ agent, version: candidate })));
    ...
    const addNode = (executorType: "skill" | "agent") => {
        let node = createWorkflowNode(editor, executorType);
        if (executorType === "skill" && skillOptions[0]) node = applySkillOptionToWorkflowNode(node, skillOptions[0]);
        if (executorType === "agent" && publishedAgents[0]) node.agentRef = { agentId: publishedAgents[0].agent.agent.id, agentVersionId: publishedAgents[0].version.id, agentVersionConstraint: "" };
        ...
        <section className="studio-panel min-w-0 overflow-hidden">
            <header className="border-b border-[var(--studio-border-subtle)] p-5">
                <div className="flex flex-wrap items-start justify-between gap-4">
                    <div className="min-w-0"><p className="text-xs font-semibold tracking-[0.16em] text-[var(--studio-accent)]">DAG DEFINITION</p><h2 className="mt-2 truncate text-2xl font-semibold">{item.workflow.name}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--studio-text-secondary)]">{item.workflow.summary || "组合独立 Skill 与 Agent，所有节点统一落到 Invocation / Artifact Runtime。"}</p></div>
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2">
                    ...
                    {!readOnly ? <><Button type="primary" icon={<Save className="size-4" />} disabled={!isDirty} loading={saveMutation.isPending} onClick={() => saveMutation.mutate()}>保存 DAG</Button><Button icon={<CheckCircle2 className="size-4" />} loading={validateMutation.isPending} onClick={() => validateMutation.mutate()}>校验</Button><Button icon={<Send className="size-4" />} loading={publishMutation.isPending} onClick={() => publishMutation.mutate()}>发布</Button></> : null}
            ...
            <div className="space-y-5 p-5">
                ...
                <div className="space-y-3">{editor.nodes.map((node, index) => <WorkflowNodeEditor key={node.nodeKey} index={index} node={node} allNodes={editor.nodes} readOnly={readOnly} skillOptions={skillOptions} agents={publishedAgents} onChange={(value) => updateNode(node.nodeKey, value)} onRemove={() => setPackage(removeWorkflowNode(editor, node.nodeKey))} />)}</div>
                {!readOnly ? <div className="flex flex-wrap gap-2"><Button icon={<Wrench className="size-4" />} onClick={() => addNode("skill")}>添加 Skill 节点</Button><Button icon={<Bot className="size-4" />} disabled={!publishedAgents.length} onClick={() => addNode("agent")}>添加 Agent 节点</Button></div> : null}
...
function WorkflowNodeEditor({ index, node, allNodes, readOnly, skillOptions, agents, onChange, onRemove }: { index: number; node: WorkflowNodeSpec; allNodes: WorkflowNodeSpec[]; readOnly: boolean; skillOptions: SkillOption[]; agents: Array<{ agent: AgentRegistryItem; version: AgentRegistryItem["versions"][number] }>; onChange: (node: WorkflowNodeSpec) => void; onRemove: () => void }) {
    ...
    const agentOptions = agents.map(({ agent, version }) => ({ value: version.id, label: `${agent.agent.name} · v${version.version}` }));
    return (
        <article className="rounded-lg border border-[var(--studio-border-subtle)] bg-[var(--studio-panel-muted-bg)] p-4">
            <div className="flex items-start gap-3"><div className="grid size-8 shrink-0 place-items-center rounded-md bg-[var(--studio-accent-soft)] text-sm font-semibold text-[var(--studio-accent)]">{index + 1}</div><div className="grid min-w-0 flex-1 gap-3 md:grid-cols-2"><Input addonBefore="名称" value={node.name} readOnly={readOnly} onChange={(event) => update({ name: event.target.value })} /><Input addonBefore="Key" value={node.nodeKey} readOnly={readOnly} onChange={(event) => update({ nodeKey: event.target.value.toLowerCase().replace(/[^a-z0-9_-]/g, "_") })} /></div><Tooltip title="移除节点"><Button aria-label="移除节点" danger icon={<Trash2 className="size-4" />} disabled={readOnly || allNodes.length === 1} onClick={onRemove} /></Tooltip></div>
            <div className="mt-3 grid gap-3 lg:grid-cols-2">
                <label><span className="mb-1 block text-xs text-[var(--studio-text-muted)]">执行器</span><Select className="w-full" value={node.executorType} disabled={readOnly} options={[{ value: "skill", label: "Skill · 单能力执行" }, { value: "agent", label: "Agent · 多 Skill 计划" }]} onChange={(executorType: "skill" | "agent") => update(executorType === "skill" ? { executorType, agentRef: undefined, skillBinding: { mode: "fixed", skillId: "", skillVersionId: "", skillVersionConstraint: "", capability: "", expectedOutputArtifactType: "", projectTags: [], candidateSkillIds: [] } } : { executorType, skillBinding: undefined, agentRef: { agentId: "", agentVersionId: "", agentVersionConstraint: "" } })} /></label>
