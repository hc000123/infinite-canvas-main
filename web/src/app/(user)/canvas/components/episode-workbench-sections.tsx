        ...
        <div className="grid gap-3 md:grid-cols-3">
            {modes.map((mode) => (
                <Card key={mode.key} size="small" className={mode.active ? "" : "opacity-60"}>
                    ...
                    <p className="mt-2 text-sm leading-6 text-[var(--studio-text-muted)]">{mode.description}</p>
                </Card>
            ))}
        </div>
    );
    ...
    return (
        ...
        >
            <div className="mb-3 flex flex-wrap items-center gap-2 text-sm">
                ...
                <span className="text-[var(--studio-text-muted)]">{hasEpisode ? "剧本快照可编辑，保存前不会覆盖分镜头。" : "未绑定剧本时仍可自由画布制作。"}</span>
            </div>
