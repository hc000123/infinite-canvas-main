        ...
        <Modal title="图片超分" open={Boolean(node)} onCancel={onClose} footer={null} width={460} centered destroyOnHidden>
            {node ? (
                <div className="space-y-4">
                    <Alert showIcon icon={<CloudUpload className="size-4" />} type="info" message="图片将上传到云端服务处理" description="API 密钥只保存在服务端。原图片节点不会被替换，结果会作为右侧新节点加入画布并归档到资产。" />
                    <div>
                    ...
                    {!enabled ? <Alert type="warning" showIcon message={capabilities ? "服务端尚未配置图片超分" : "暂时无法确认服务端图片超分配置"} description="请检查后端服务，并在后端环境变量中配置阿里云 AccessKey。" /> : null}
