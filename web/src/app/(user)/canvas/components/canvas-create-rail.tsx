    ...
    return (
        <div className="pointer-events-none absolute bottom-24 left-4 top-16 z-50 flex items-center">
            <nav data-canvas-editorial-surface aria-label="左侧创建栏" className="pointer-events-auto relative flex flex-col gap-0.5 rounded-md border p-1" style={{ background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.toolbar.item }}>
                <CanvasToolButton instantFeedback label="选择" icon={<MousePointer2 className="size-4.5" />} onClick={actions.onDeselect} />
                <CanvasToolButton instantFeedback label="文本" icon={<FileType2 className="size-4.5" />} onClick={actions.onAddText} />
                <CanvasToolButton instantFeedback label="图片" icon={<ImageIcon className="size-4.5" />} onClick={actions.onAddImage} />
                <CanvasToolButton instantFeedback label="视频" icon={<Video className="size-4.5" />} onClick={actions.onAddVideo} />
                <CanvasToolButton instantFeedback label="音频" icon={<AudioLines className="size-4.5" />} onClick={actions.onAddAudio} />
                <Dropdown trigger={["click"]} placement="bottomLeft" open={moreOpen} onOpenChange={setMoreOpen} menu={{ items: moreItems }}>
                    <button
                        type="button"
                        className="group relative grid h-8 w-8 place-items-center transition-colors duration-75 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--studio-focus-ring)]"
                        style={{ color: moreOpen ? theme.accent : theme.toolbar.item }}
                        aria-label="更多"
                        aria-haspopup="menu"
                        aria-expanded={moreOpen}
                    >
                        <span className="grid size-8 place-items-center rounded-md transition-colors duration-75" style={{ background: moreOpen ? theme.toolbar.activeBg : undefined, outline: moreOpen ? `1px solid ${theme.focusRing}` : undefined }}>
