...
export function CanvasConfigNodePanel({ node, canvasAiConfig, isRunning, inputSummary, inputs, onConfigChange, onTextInputChange, onGenerate, onPreviewReference, connectedMedia = [], onDisconnectConnectedMedia }: CanvasConfigNodePanelProps) {
    ...
    const credits = requestCreditCost({ channelMode: config.channelMode, modelCosts, model: config.model, fallbackModel: mode === "video" ? config.seedanceModel || config.videoModel : undefined, count: mode === "video" ? requestCreditQuantity({ count: config.videoSeconds, videoProtocol: config.videoProtocol, videoModel: config.seedanceModel || config.videoModel || config.model, videoTaskMode: config.videoTaskMode }) : mode === "image" ? count : 1 });
