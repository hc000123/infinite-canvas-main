...
import { RecoverableVideoTaskError, requestVideoGeneration, type NormalizedVideoTask, type VideoGenerationReferences } from "@/services/api/video";
...
export async function runCanvasVideoGeneration(config: AiConfig, prompt: string, references: VideoGenerationReferences, onStatus?: (task: NormalizedVideoTask) => void, trace?: AiTaskTrace) {
    ...
    let videoBlob: Blob;
    try {
        videoBlob = await requestVideoGeneration(config, prompt, references, {
            onStatus: (task) => {
                completedTask = task;
                onStatus?.(task);
            },
    ...
    const video = (await uploadMediaFile(videoBlob, "video")) as CanvasUploadedVideo;
