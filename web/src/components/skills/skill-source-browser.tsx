...
export function SkillSourceBrowser({ token, versionId }: { token: string; versionId: string }) {
    const [path, setPath] = useState("");
    const files = useQuery({ queryKey: ["admin", "skill-source", versionId, token], queryFn: () => fetchAdminSkillSourceFiles(token, versionId), enabled: Boolean(token && versionId), retry: false });
    ...
    const content = useQuery({ queryKey: ["admin", "skill-source-text", versionId, path, token], queryFn: () => fetchAdminSkillSourceText(token, versionId, path), enabled: Boolean(token && versionId && path), retry: false });
