...
import { diffSkillFolderFiles, readDroppedSkillFolder, readSkillFolderMetadata, type SkillFolderDiff, type SkillFolderMetadata } from "./skill-folder-import-utils";
...
export function SkillFolderImport({ open, token, scope = "admin", projectId, skillId, previousVersionId, onCancel, onImported }: SkillFolderImportProps) {
    ...
    const mutation = useMutation<SkillFolderImportResult>({
        mutationFn: (): Promise<SkillFolderImportResult> => updating
            ? scope === "admin" ? importAdminSkillFolderVersion(token, skillId!, files, fields.version.trim() || undefined) : importProjectSkillFolderVersion(token, skillId!, files, fields.version.trim() || undefined)
    ...
    const acceptFiles = async (nextFiles: File[]) => {
        setFiles(nextFiles);
        setDiff(undefined);
        if (!nextFiles.length) return;
        try {
            setFields(await readSkillFolderMetadata(nextFiles, updating ? "" : "1.0.0"));
        } catch (error) {
            setFields(emptyFields);
    ...
    const handleDrop = async (event: React.DragEvent<HTMLButtonElement>) => {
        ...
        setReading(true);
        try {
            await acceptFiles(await readDroppedSkillFolder(Array.from(event.dataTransfer.items)));
        } catch (error) {
            message.warning(error instanceof Error ? error.message : "无法读取拖入的文件夹");
        } finally {
            setReading(false);
        }
    };
    const waitingForDiff = updating && files.length > 0 && (!previousVersionId || previousFiles.isLoading || diffing || !diff);
    const disabled = !files.length || !hasSkill || (!updating && (!stageKey || !fields.name.trim() || !fields.version.trim())) || waitingForDiff || Boolean(diffError || previousFiles.error);
    ...
    return <Modal width={720} title={updating ? "载入 Skill 新版本" : "载入外部 Skill 文件夹"} open={open} onCancel={onCancel} footer={<Flex justify="space-between" align="center" gap={16}><Typography.Text type="secondary" className="text-xs">系统会冻结完整文件夹，不执行其中脚本。</Typography.Text><Flex gap={8}><Button onClick={onCancel}>取消</Button><Button type="primary" loading={mutation.isPending} disabled={disabled} onClick={() => mutation.mutate()}>载入并创建草稿</Button></Flex></Flex>}>
        <Flex vertical gap={16}>
            ...
            <input ref={inputRef} hidden type="file" multiple onChange={(event) => { void acceptFiles(Array.from(event.target.files || [])); event.target.value = ""; }} />
            ...
            {updating && files.length ? <FileDiff diff={diff} loading={previousFiles.isLoading || diffing} error={diffError || (previousFiles.error instanceof Error ? previousFiles.error.message : previousFiles.error ? "读取上一版文件索引失败" : !previousVersionId ? "请先选择要对比的当前版本" : "")} /> : null}
        </Flex>
    </Modal>;
}
