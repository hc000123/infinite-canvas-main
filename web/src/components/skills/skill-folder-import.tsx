...
export function SkillFolderImport({ open, token, skillId, previousVersionId, onCancel, onImported }: SkillFolderImportProps) {
    ...
    const mutation = useMutation<SkillFolderImportResult>({
        ...
        onError: (error) => { setPreparing(false); setReading(false); message.error(error instanceof Error ? error.message : "导入失败"); },
    });

    const beginPreparing = (recursive: boolean) => {
        const request = requestGuard.current.begin();
        setPreparing(true);
        setReading(recursive);
        setFiles([]);
        setFields(emptyFields);
        setDiff(undefined);
        setDiffError("");
    ...
    const acceptFiles = async (nextFiles: File[], request: ReturnType<typeof requestGuard.current.begin>) => {
        ...
        if (!nextFiles.length) { setPreparing(false); return; }
        try {
    ...
    const disabled = !canSubmitSkillFolderImport({
        ...
        name: fields.name,
        preparing,
        hasPreviousVersion: Boolean(previousVersionId),
        previousFilesLoading: previousFiles.isFetching,
        hasComparableBaseline: previousFiles.data !== undefined,
        diffing,
        diffReady: Boolean(diff),
        baselineUnavailable: Boolean(previousFiles.error || !previousVersionId),
